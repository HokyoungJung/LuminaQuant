...
- broad `omx team` 실행 전 leader workspace는 깨끗해야 합니다. 생성된 context update가
  있다면 worker worktree를 열기 전에 로컬 commit으로 고정합니다.
- worker worktree 변경은 leader가 검증 gate, changed-file deslop review,
  staged diff check를 모두 확인하기 전까지 merge/push하지 않습니다.
