...
- GPU 활성화 전 extras 설치:
- `uv sync --extra gpu` (`cudf-polars-cu13>=26.8,<27`,
  `nvidia-cuda-nvrtc>=13.2,<14`로 pin)
- 의존성 업데이트는 cuDF가 지원하는 Polars 범위를 유지해야 합니다. Pandas 3가
  기준입니다. Polars/cuDF 범위 변경은 별도 GPU/parity migration으로 다루고
