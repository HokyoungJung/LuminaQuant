...
- Operator update: strict `6.0x` is now the leading research/shadow candidate despite the deployable gate failure; the `13.6667%` OOS MDD is considered tolerable for follow-up, but not sufficient by itself to set `deployable_success=true`.
