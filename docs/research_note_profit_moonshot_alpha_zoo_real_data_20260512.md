...
Completed a repo-wide inventory and policy audit for hybrid v3.5/v3.6, hybrid Optuna, tuning/optimization, candidate-hybrid, calendar Optuna, and fresh-portfolio optimization artifacts against the preserved private/main baseline `1c6816fced44d277f6c7112934c9dded65ba710f`. Corrections on 2026-05-17 KST: the **strict core universe excludes every calendar/current-base-derived row and every nested-hybrid/same-family/opaque hybrid-tuning row before ranking**. Calendar/current-base and nested-hybrid rows are retained only in quarantine/reference ledgers.
...
- Hybrid v3.5/v3.6 rows are **not strict-core rows** after correction: the inspected candidates are nested hybrid/portfolio/allocator/leverage-sweep/static-blend composites, so they are retained only in `excluded_nested_hybrid_same_family_quarantine_latest.csv`.
...
- State-distilled fresh portfolio tuning is a same-family parameter-clone portfolio, not an independent hybrid universe member; it is quarantine/reference only.
...
- Candidate-hybrid had strong OOS metrics but is excluded due calendar/current-base-source dependency, validation liquidation count `1`, and the no-hybrid-inside-hybrid composition rule.
...
- Strict core split performance CSV: `var/reports/profit_moonshot_20260501/current_tail_20260508/alpha_v2/hybrid_optuna_alpha_zoo_comparison_20260514/candidate_split_performance_latest.csv`
