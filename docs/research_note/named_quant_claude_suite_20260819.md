...
- **배선**: `allocate_quality_gated(..., risk_scaling=...)`이 모든 틸트/투영 **뒤에** L을 곱하고(상대 구조 보존, 미지정 시 바이트 동일), `build_allocation_manifest`가 spec+계산치(`exposure`/`sigma_annual`/`cash_weight`)를 `risk_scaling` provenance로 기록, CLI(`build_quality_gated_allocation.py`)는 실행 가능한 키만 담은 입력 JSON의 `risk_scaling` 블록을 그대로 전달한다. 설명과 사전등록 변형은 별도 `risk_scaling_research` 블록에 있으며, `compare_hierarchical_allocators.py`는 method별 L·cash와 Kelly μ-민감도를 함께 보고한다. 퇴화 입력(σ̂=0, 관측 부족, 공분산-부재 폴백)은 전부 fail-closed.
...
- **호스트 TZ 의존 전수 제거(완료)**: 독립 감사 스윕(3렌즈·2표 검증)이 확인한 naive-datetime 로컬TZ 해석 지점을 전부 naive=UTC로 통일했다 — 엔진 게이트 `_event_time_to_ms`, 이벤트 identity `event_clock`, `market_data` 증분 동기화 커서(`get_last_timestamp_ms`/`_1s`: KST면 9h 중복 재수집, UTC 서쪽 호스트면 **조용한 갭**), `services/materialize_from_raw`(end_date 경계·파티션 `window_start/end_ms`), `timeframe_aggregator._coerce_timestamp_ms`, `live/data_external` 외부 parquet 피드, `optimization/frozen_dataset._as_ns_epoch`, `data_sync` 통계, `research_runner` 창 간격, `live/readiness_policy._parse_utc`, `utils/report_generator`(UTC 스탬프). 정본 헬퍼는 `lumina_quant/utils/timeutil.py`(`as_utc`/`utc_epoch_seconds`/`utc_epoch_ms`: naive==UTC, 숫자는 <1e11이면 초). 회귀테스트는 TZ 3종(UTC/Asia/Seoul/America/New_York) 파라미터: `tests/test_timeframe_gate_naive_utc.py`, `tests/test_market_data_cursor_naive_utc.py`(실제 임시 리포지토리로 커서 검증). 남은 `.timestamp()` 호출은 전부 tz-aware 가드 뒤에 있음을 확인했다. 따라서 data-PC를 `TZ=UTC`로 띄우는 것은 더 이상 필수가 아니지만, 기존 KST 해석으로 쓰인 로컬 캐시가 있다면 첫 동기화에서 9h 겹침 재수집(중복은 upsert로 흡수)만 예상하면 된다.
