...
- **data-PC 호스트 TZ 권고(`TZ=UTC`)**: 엔진 게이트·이벤트 identity는 수정했지만, 독립 감사 스윕에서 naive datetime을 호스트 로컬TZ로 해석하는 지점이 데이터 계층에 더 남아 있음을 확인했다(전부 확인됨): ~~`market_data.py` 증분 동기화 커서 `get_last_timestamp_ms`/`get_last_ohlcv_1s_timestamp_ms`~~ (**수정·테스트 완료**: naive=UTC `_datetime_to_epoch_ms` 사용), ~~`services/materialize_from_raw.py` `_coerce_timestamp_ms`(end_date 경계)~~ (**수정 완료**), `services/materialize_from_raw.py` `window_start/end_ms` 파생(~L175, high — 미수정), `timeframe_aggregator._coerce_timestamp_ms`(~L57-73, medium), `live/data_external.py` 외부 parquet 피드(~L200, medium), `eval/frozen_dataset._as_ns_epoch`, `data_sync` 통계, `live/readiness_policy._parse_utc`(low). KST 호스트에서 동기화 커서/물질화 창이 9시간 어긋날 수 있으므로 **근본 수정 전까지 data-PC 파이프라인(동기화·물질화·러너 전부)을 `TZ=UTC` 환경으로 실행**하고, 수정은 기존 로컬 캐시와의 규약 충돌(중복/갭)을 점검한 뒤 한 번에 적용할 것.
