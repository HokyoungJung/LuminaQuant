...
- Real official-archive smoke backfill succeeded for `TRXUSDT` on `2024-03-30`: the archive contained `722,198` raw rows, cadence-sampled to `288` five-minute rows, and persisted `288` BBO feature rows. Support inventory now shows `TRXUSDT 988` BBO rows from `2024-03-30T00:00:00.007Z` through `2026-06-07T10:31:00Z`.
- This still does **not** change the cap-500 clean OOS result or permit real-money deployment. The next required step is bounded official-archive backfill across enough core symbols/folds, then a fresh train/validation-first BBO-aware clean rerun.
