...
- Follow-up implementation: added `scripts/import_binance_book_ticker_history.py` plus tests so an explicitly approved external Binance USDⓈ-M BBO history file (`csv`, `jsonl`/`ndjson`, or `parquet`) can be normalized into the existing feature-point store with Binance `bookTicker` aliases (`b`/`B`/`a`/`A`) preserved. This only removes an ingestion plumbing blocker; it does **not** approve any vendor/source, does **not** alter the cap-500 clean OOS result, and does **not** permit real-money deployment without a new clean train/validation-first walk-forward rerun on approved historical coverage.
