"""Run a fail-closed registry strategy screen over canonical one-minute bars.

The normal ``lq backtest`` CLI is optimized for the current 1s/raw-first execution
contract. This first-stage filter provides bounded 1m research validation
over the canonical local parquet partitions. This script keeps the actual
...
def _run_backtest(
    *,
    ...
    runtime_config = (
        get_default_runtime_config() if config_path is None else load_runtime_config(config_path)
    )
...
def _write_outputs(
    ...
    *,
    ...
    json_path = output_dir / f"strategy_screen_{timestamp}.json"
...
def run_strategy_screen(args: argparse.Namespace) -> dict[str, Any]:
            ...
            strategy_results.append(
                {
                    ...
                    "error": message,
                    "exclusion_reason": (
                        "canonical OHLCV window unavailable; no bars were inferred, filled, or synthesized"
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--config",
        type=Path,
        help="Explicit RuntimeConfig; rigorous stages should use a realistic-cost profile.",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    payload = run_strategy_screen(args)
