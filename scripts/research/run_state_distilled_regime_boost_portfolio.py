...
DEFAULT_PANEL_PARQUET = Path("var/cache/profit_moonshot_fresh_start/joined_panel_de62df511cec53df6ad39521.parquet")
...
def split_labels(times: list[Any], *, train_fraction: float = 0.6, validation_fraction: float = 0.2) -> dict[Any, str]:
    unique = sorted(set(times))
    if not unique:
        return {}
    train_cut = max(1, math.floor(len(unique) * train_fraction))
    validation_cut = min(len(unique), max(train_cut + 1, math.floor(len(unique) * (train_fraction + validation_fraction))))
    out: dict[Any, str] = {}
...
def build_feature_rows(rows: list[dict[str, Any]], config: RegimeBoostConfig) -> list[dict[str, Any]]:
    n = len(rows)
    ...
    long_vol = {sym: rolling_vol(ret1[sym], 720) * math.sqrt(ANNUALIZATION_HOURS) for sym in SYMBOLS}

    btc_ret_z = zscore(ret168["BTCUSDT"])
    eth_ret_z = zscore(ret168["ETHUSDT"])
    btc_rv_ratio = np.divide(rv72["BTCUSDT"], rv720["BTCUSDT"], out=np.ones(n), where=np.abs(rv720["BTCUSDT"]) > 1e-12)
    ...
    risk_score = np.asarray([safe_float(row.get("external_risk_off_score_lag1"), 0.0) for row in rows], dtype=float)
    usd_z = np.asarray([safe_float(row.get("external_usd_ret_z_lag1"), 0.0) for row in rows], dtype=float)
    vix_z = np.asarray([safe_float(row.get("external_vix_z_lag1"), 0.0) for row in rows], dtype=float)
    curve_z = np.asarray([safe_float(row.get("external_curve_z_lag1"), 0.0) for row in rows], dtype=float)
    oil_abs_z = np.abs(np.asarray([safe_float(row.get("external_wti_ret_z_lag1"), 0.0) for row in rows], dtype=float))
    cfg = config.regime
    composed_risk = (
    ...
    for idx, row in enumerate(rows):
        ...
        for sym in ALT_SYMBOLS:
            symbol_scores[sym] = (
                config.signal_residual_weight * safe_float(residual_z[sym][idx])
        ...
        enriched.update(
            {
                ...
                "next_highs": {sym: safe_float(highs[sym][idx + 1]) if idx + 1 < n else safe_float(highs[sym][idx]) for sym in SYMBOLS},
                "next_lows": {sym: safe_float(lows[sym][idx + 1]) if idx + 1 < n else safe_float(lows[sym][idx]) for sym in SYMBOLS},
...
def classify_regime(
    *,
    ...
    if trend_score < config.trend_bear_threshold or risk_score > config.risk_bear_threshold:
        return "bear"
    if abs(trend_score) <= config.trend_bull_threshold and dispersion_score > config.dispersion_threshold:
...
def effective_dynamic_leverage(
    *,
    ...
    max_lev = min(config.max_effective_leverage, requested_max_leverage or config.max_effective_leverage)
    ...
    elif confidence > local_config.high_confidence_threshold and vol_ratio < local_config.low_vol_ratio_threshold:
        requested = max_lev
    ...
    return float(max(local_config.min_effective_leverage, min(requested, asset_volatility_cap(long_term_vol, local_config), max_lev)))
...
def grid_product(config: RegimeBoostConfig) -> list[RegimeBoostConfig]:
    ...
    for core_scale, base_leverage, entry_threshold, bull_long, bear_short, booster_weight, booster_cap, pair_weight, stride in itertools.product(
        core_scales, base_leverages, entry_thresholds, bull_longs, bear_shorts, booster_weights, booster_caps, pair_weights, strides
    ):
        configs.append(
            replace(
                config,
                ...
                side_bias=replace(config.side_bias, bull_long_multiplier=bull_long, bear_short_multiplier=bear_short),
                leverage=replace(config.leverage, base_leverage=base_leverage),
                booster=replace(config.booster, allocation_weight=booster_weight, max_leverage=booster_cap),
...
def enforce_grid_cap(configs: list[RegimeBoostConfig], selection: SelectionConfig) -> tuple[list[RegimeBoostConfig], dict[str, Any]]:
...
def fit_neutral_pair_overlay(rows: list[dict[str, Any]], config: NeutralPairOverlayConfig) -> dict[str, Any]:
    fit_rows = [row for row in rows if str(row.get("split")) in set(config.fit_splits)]
    if not fit_rows:
        return {"eligible_symbols": [], "hedge_ratios": {}, "fit_splits": list(config.fit_splits), "as_of_policy": "no_fit_rows"}
    returns = {sym: np.asarray([safe_float(row["next_returns"].get(sym)) for row in fit_rows], dtype=float) for sym in ALT_SYMBOLS}
    ...
    for long_sym, short_sym in itertools.permutations(eligible, 2):
        ...
        hedge_ratios[f"{long_sym}|{short_sym}"] = float(max(config.hedge_ratio_floor, min(config.hedge_ratio_ceiling, ratio)))
    return {
...
def evaluate_config(
    ...
    *,
    ...
    for idx, row in enumerate(rows[:-1]):
        ...
        core_a_long_weight = config.core_a_weight * (config.external_risk_downshift if risk_score > config.regime.risk_bear_threshold else 1.0)
        core_a_short_weight = config.core_a_weight * (config.external_risk_short_boost if risk_score > config.regime.risk_bear_threshold else 1.0)
        ...
        if config.booster.allocation_weight > 0.0:
            booster_side = "short" if regime == "bear" and abs(laggard_score) >= abs(leader_score) else "long"
            booster_symbol = laggard if booster_side == "short" else leader
            if booster_allowed(row, config, side=booster_side):
                lev = leg_leverage(booster_symbol, diagnostic_max_leverage or config.booster.max_leverage)
        ...
        maintenance = max(0.0, equity_estimate * gross_notional * config.margin.maintenance_margin_rate)
        ...
        metrics[split].update(
            {
                ...
                "margin_buffer_positive": float(min(split_buffers.get(split, [STARTING_EQUITY]))) > 0.0,
...
def selection_score(metrics: dict[str, dict[str, Any]], config: SelectionConfig) -> float:
    ...
    liq_penalty = (int(train.get("liquidation_count", 0)) + int(val.get("liquidation_count", 0))) * config.liquidation_penalty
    score = (
        ...
        - config.drawdown_penalty * (safe_float(train.get("max_drawdown")) + safe_float(val.get("max_drawdown")))
        - liq_penalty
    )
...
def strict_lane_promoted(metrics: dict[str, dict[str, Any]], selection: SelectionConfig) -> tuple[bool, list[str]]:
    reasons: list[str] = []
    for split in ("train", "validation", "locked_oos"):
...
def run_selection(rows: list[dict[str, Any]], config: RegimeBoostConfig) -> tuple[dict[str, Any], list[dict[str, Any]], dict[str, Any], dict[str, Any]]:
        ...
        ledger.append(
            {
                ...
                "booster_activation_count_train_validation": eval_result["booster_activation_count"],
                "pair_overlay_activation_count_train_validation": eval_result["pair_overlay_activation_count"],
            }
        )
    selected = select_candidate_from_rows(ledger)
    ledger_sorted = sorted(ledger, key=lambda row: safe_float(row.get("selection_score")), reverse=True)
...
def write_freeze_artifacts(output_dir: Path, freeze_payload: dict[str, Any]) -> tuple[Path, Path, str]:
...
def diagnostic_leverage_sweep(rows: list[dict[str, Any]], config: RegimeBoostConfig, pair_fit: dict[str, Any]) -> list[dict[str, Any]]:
    out: list[dict[str, Any]] = []
    for cap in (5.0, 6.0, 10.0, 15.0, 25.0):
        diag_config = replace(
            config,
            booster=replace(config.booster, enabled=True, allocation_weight=max(config.booster.allocation_weight, 0.10), max_leverage=cap),
            leverage=replace(config.leverage, max_effective_leverage=min(25.0, max(config.leverage.max_effective_leverage, cap))),
        )
        result = evaluate_config(rows, diag_config, pair_fit, include_locked_oos=True, diagnostic_max_leverage=cap)
        split_status = result["split_metrics"]
        total_liq = sum(int(split_status[split].get("liquidation_count") or 0) for split in split_status)
        min_buffer = min(safe_float(split_status[split].get("minimum_margin_buffer"), STARTING_EQUITY) for split in split_status)
        out.append(
            {
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if args.grid_limit is not None:
        config = replace(config, selection=replace(config.selection, grid_limit=int(args.grid_limit)))
    if args.max_booster_leverage is not None:
        ...
        config = replace(
            ...
            leverage=replace(config.leverage, max_effective_leverage=min(25.0, max(config.leverage.max_effective_leverage, lev_cap))),
        )

    rows = build_feature_rows(load_market_rows(Path(args.panel_parquet), Path(args.external_state_csv)), config)
    selected, ledger, grid_meta, pair_fit = run_selection(rows, config)
    ledger_path = output_dir / "selection_ledger.jsonl"
    write_ledger(ledger_path, ledger)
    input_manifest = build_input_manifest([Path(args.panel_parquet), Path(args.external_state_csv), *[Path(p) for p in args.candidate_artifacts]])
    ...
    gate = build_locked_oos_gate(freeze_path=freeze_path, freeze_hash=freeze_hash, frozen_config=frozen_config, rows=rows, pair_fit=pair_fit)
    ...
    total_liq = sum(int(strict_metrics[split].get("liquidation_count") or 0) for split in strict_metrics)
    min_buffer = min(safe_float(strict_metrics[split].get("minimum_margin_buffer"), STARTING_EQUITY) for split in strict_metrics)
    ...
    summary = {
        ...
        "locked_oos_gate": {
            ...
            "uses_locked_oos_for_selection": False,
            "selected_params_byte_identical_to_freeze": stable_json_hash(gate["selected_config"]) == stable_json_hash(freeze_payload["selected_config"]),
        },
        "strict_lane": {
    ...
    timestamped_summary_path = output_dir / f"state_distilled_regime_boost_summary_{datetime.now(UTC).strftime('%Y%m%dT%H%M%SZ')}.json"
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--candidate-artifacts", nargs="*", default=[str(p) for p in DEFAULT_CANDIDATE_ARTIFACTS])
...
def main(argv: list[str] | None = None) -> int:
    ...
    summary = run(args)
    print(json.dumps({"summary_path": summary["artifact_paths"]["summary_latest"], "strict_promoted_success": summary["strict_lane"]["promoted_success"]}, sort_keys=True))
    return 0
