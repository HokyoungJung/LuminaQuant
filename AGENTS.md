...
- **BacktestConfigView (DELETION-GATE: Phase 5)**: still in use as an uppercase-attr bridge for the backtest engine and CLI. `Backtest()` auto-wraps a bare `RuntimeConfig` in `BacktestConfigView`; `BacktestConfigView._rt` holds the source `RuntimeConfig`. New code should pass `RuntimeConfig` directly; do NOT create `BacktestConfigView` manually in new code.
