...
def _rolling_beta(a: deque[float], b: deque[float], window: int, beta_floor: float, beta_cap: float) -> float | None:
...
def _spread_zscore(pair: _PairState, spread_window: int, beta: float) -> tuple[float | None, float | None]:
...
class DonchianAtrTrendStrategy(_StatePackingMixin, Strategy):
    ...
    def get_param_schema(cls) -> dict[str, HyperParam]:
        return {
            "breakout_lookback": HyperParam.integer("breakout_lookback", default=120, low=4, high=10080),
            ...
            "entry_atr_buffer": HyperParam.floating("entry_atr_buffer", default=0.10, low=0.0, high=5.0),
            "stop_atr_multiple": HyperParam.floating("stop_atr_multiple", default=2.5, low=0.0, high=20.0),
            "trail_atr_multiple": HyperParam.floating("trail_atr_multiple", default=3.0, low=0.0, high=20.0),
            "min_trend_efficiency": HyperParam.floating("min_trend_efficiency", default=0.18, low=0.0, high=1.0),
            ...
            "max_order_value": HyperParam.floating("max_order_value", default=300.0, low=0.0, high=1_000_000.0, tunable=False),
    ...
    def __init__(self, bars: Any, events: Any, **params: Any) -> None:
        ...
        size = max(self.breakout_lookback, self.exit_lookback, self.atr_window, self.max_hold_bars) + 8
    ...
    def _maybe_exit(self, symbol: str, item: _ChannelState, snapshot: _Snapshot, prior_mid: float | None, atr: float | None) -> None:
        close = safe_float(snapshot.close)
    ...
    def _process_symbol(self, symbol: str, snapshot: _Snapshot) -> None:
        ...
        atr = average_true_range(item.tr_values, self.atr_window) if len(item.tr_values) >= self.atr_window else None
        self._maybe_exit(symbol, item, snapshot, prior_mid, atr)
        if item.mode == "OUT" and ready and atr is not None and prior_upper is not None and prior_lower is not None:
            eff = trend_efficiency(item.closes, window=min(self.breakout_lookback, max(3, self.atr_window)))
            vr = volatility_ratio(item.closes, fast_window=max(2, self.atr_window // 2), slow_window=max(self.atr_window + 1, self.breakout_lookback))
            if (
                eff is not None
                ...
                if signal_type:
                    ...
                    if self.stop_atr_multiple > 0.0:
                        stop_loss = close - self.stop_atr_multiple * atr if signal_type == "LONG" else close + self.stop_atr_multiple * atr
                    ...
                    _emit(
                        ...
                        strength=max(0.25, min(3.0, abs(close - (prior_upper if signal_type == "LONG" else prior_lower)) / max(atr, _EPS))),
                        price=close,
                        stop_loss=stop_loss,
...
class VolatilitySqueezeBreakoutStrategy(_StatePackingMixin, Strategy):
    ...
    def get_param_schema(cls) -> dict[str, HyperParam]:
        return {
            "compression_window": HyperParam.integer("compression_window", default=48, low=4, high=4096),
            "baseline_window": HyperParam.integer("baseline_window", default=240, low=16, high=20000),
            "breakout_lookback": HyperParam.integer("breakout_lookback", default=48, low=4, high=4096),
            "range_ratio_max": HyperParam.floating("range_ratio_max", default=0.65, low=0.05, high=2.0),
            "min_volume_z": HyperParam.floating("min_volume_z", default=0.5, low=-5.0, high=20.0),
            "momentum_lookback": HyperParam.integer("momentum_lookback", default=12, low=1, high=4096),
            "momentum_threshold": HyperParam.floating("momentum_threshold", default=0.001, low=0.0, high=1.0),
            "atr_window": HyperParam.integer("atr_window", default=48, low=2, high=4096),
            "stop_atr_multiple": HyperParam.floating("stop_atr_multiple", default=2.0, low=0.0, high=20.0),
            "trail_atr_multiple": HyperParam.floating("trail_atr_multiple", default=2.5, low=0.0, high=20.0),
            "allow_short": HyperParam.boolean("allow_short", default=True, grid=[True, False]),
    ...
    def __init__(self, bars: Any, events: Any, **params: Any) -> None:
        ...
        self.max_order_value = max(0.0, float(resolved["max_order_value"]))
        self.min_price = max(0.0, float(resolved["min_price"]))
        size = max(self.baseline_window, self.breakout_lookback, self.atr_window, self.max_hold_bars) + 8
        self._state = {symbol: _ChannelState(deque(maxlen=size), deque(maxlen=size), deque(maxlen=size), deque(maxlen=size), deque(maxlen=size), deque(maxlen=size)) for symbol in self.symbol_list}
    ...
    def _exit_if_needed(self, symbol: str, item: _ChannelState, snapshot: _Snapshot, atr: float | None) -> None:
        close = safe_float(snapshot.close)
        if item.mode not in {"LONG", "SHORT"} or item.entry_price is None or close is None:
            return
        item.bars_held += 1
        ...
        if item.mode == "LONG":
            item.high_watermark = max(float(item.high_watermark or close), close)
            if self.stop_atr_multiple > 0.0 and close <= item.entry_price - self.stop_atr_multiple * atr_value:
                reason = "atr_stop"
            elif self.trail_atr_multiple > 0.0 and close <= item.high_watermark - self.trail_atr_multiple * atr_value:
                reason = "atr_trail"
        else:
            item.low_watermark = min(float(item.low_watermark or close), close)
            if self.stop_atr_multiple > 0.0 and close >= item.entry_price + self.stop_atr_multiple * atr_value:
                reason = "atr_stop"
            elif self.trail_atr_multiple > 0.0 and close >= item.low_watermark + self.trail_atr_multiple * atr_value:
                reason = "atr_trail"
        if not reason and item.bars_held >= self.max_hold_bars:
            reason = "max_hold"
        if not reason:
            return
        _emit(self.events, strategy_id="volatility_squeeze_breakout", symbol=symbol, event_time=snapshot.time, signal_type="EXIT", price=close, metadata={"strategy": "VolatilitySqueezeBreakoutStrategy", "reason": reason})
        item.mode = "OUT"
        item.entry_price = None
        item.entry_atr = None
        item.high_watermark = None
    ...
    def _process_symbol(self, symbol: str, snapshot: _Snapshot) -> None:
        ...
        if close is None or close <= self.min_price:
            return
        ready = len(item.ranges) >= self.baseline_window and len(item.highs) >= self.breakout_lookback and len(item.tr_values) >= self.atr_window
        prior_upper = max(list(item.highs)[-self.breakout_lookback :]) if ready else None
        prior_lower = min(list(item.lows)[-self.breakout_lookback :]) if ready else None
        ...
        _append_market_state(item, snapshot)
        if item.mode != "OUT" or not ready or prior_upper is None or prior_lower is None or atr is None:
            return
        if short_range is None or base_range is None or base_range <= _EPS:
            return
        if short_range / base_range > self.range_ratio_max:
            return
        vol_z = volume_zscore(item.volumes, window=min(self.baseline_window, max(8, self.compression_window)))
        ...
        if close > prior_upper and mom is not None and mom >= self.momentum_threshold:
            signal_type = "LONG"
        elif self.allow_short and close < prior_lower and mom is not None and mom <= -self.momentum_threshold:
            signal_type = "SHORT"
        if not signal_type:
            return
        stop_loss = None
        ...
        _emit(self.events, strategy_id="volatility_squeeze_breakout", symbol=symbol, event_time=snapshot.time, signal_type=signal_type, strength=max(0.25, min(3.0, abs(mom) / max(self.momentum_threshold, _EPS))), price=close, stop_loss=stop_loss, trailing_percent=trailing_percent, metadata=metadata)
        item.mode = signal_type
        item.entry_price = close
        item.entry_atr = atr
        item.high_watermark = close
...
class TakerFlowImbalanceContinuationStrategy(Strategy):
    ...
    def get_param_schema(cls) -> dict[str, HyperParam]:
        return {
            ...
            "price_confirm_bars": HyperParam.integer("price_confirm_bars", default=6, low=1, high=1440),
            "imbalance_entry": HyperParam.floating("imbalance_entry", default=0.22, low=0.0, high=1.0),
            "imbalance_exit": HyperParam.floating("imbalance_exit", default=0.04, low=0.0, high=1.0),
            "min_price_move": HyperParam.floating("min_price_move", default=0.001, low=0.0, high=1.0),
            "min_volume_z": HyperParam.floating("min_volume_z", default=-0.5, low=-5.0, high=20.0),
            "volume_window": HyperParam.integer("volume_window", default=96, low=8, high=4096),
            "stop_loss_pct": HyperParam.floating("stop_loss_pct", default=0.018, low=0.0, high=0.50),
            "take_profit_pct": HyperParam.floating("take_profit_pct", default=0.045, low=0.0, high=1.0),
            "allow_short": HyperParam.boolean("allow_short", default=True, grid=[True, False]),
    ...
    def __init__(self, bars: Any, events: Any, **params: Any) -> None:
        ...
        self.max_order_value = max(0.0, float(resolved["max_order_value"]))
        self.min_price = max(0.0, float(resolved["min_price"]))
        size = max(self.flow_window, self.price_confirm_bars, self.volume_window, self.max_hold_bars) + 8
        self._state = {symbol: _FlowState(deque(maxlen=size), deque(maxlen=size), deque(maxlen=size), deque(maxlen=size), deque(maxlen=size)) for symbol in self.symbol_list}
    ...
    def _exit_if_needed(self, symbol: str, item: _FlowState, snapshot: _Snapshot, imbalance: float | None) -> None:
        close = safe_float(snapshot.close)
        if item.mode not in {"LONG", "SHORT"} or item.entry_price is None or close is None:
            return
        item.bars_held += 1
        ...
        if not reason:
            return
        _emit(self.events, strategy_id="taker_flow_imbalance_continuation", symbol=symbol, event_time=snapshot.time, signal_type="EXIT", price=close, metadata={"strategy": "TakerFlowImbalanceContinuationStrategy", "reason": reason, "imbalance": imbalance})
        item.mode = "OUT"
        item.entry_price = None
        item.bars_held = 0
    ...
    def _process_symbol(self, event: Any, symbol: str, snapshot: _Snapshot) -> None:
        ...
        if imbalance >= self.imbalance_entry and ret >= self.min_price_move:
            signal_type = "LONG"
        elif self.allow_short and imbalance <= -self.imbalance_entry and ret <= -self.min_price_move:
            signal_type = "SHORT"
        if not signal_type:
            return
        stop_loss = None
        ...
        if self.stop_loss_pct > 0.0:
            stop_loss = close * (1.0 - self.stop_loss_pct if signal_type == "LONG" else 1.0 + self.stop_loss_pct)
        if self.take_profit_pct > 0.0:
            take_profit = close * (1.0 + self.take_profit_pct if signal_type == "LONG" else 1.0 - self.take_profit_pct)
        metadata = _target_metadata(
        ...
        _emit(self.events, strategy_id="taker_flow_imbalance_continuation", symbol=symbol, event_time=snapshot.time, signal_type=signal_type, strength=max(0.25, min(3.0, abs(imbalance) / max(self.imbalance_entry, _EPS))), price=close, stop_loss=stop_loss, take_profit=take_profit, metadata=metadata)
        item.mode = signal_type
        item.entry_price = close
        item.bars_held = 0
...
class PairsSpreadMeanReversionStrategy(Strategy):
    ...
    def get_param_schema(cls) -> dict[str, HyperParam]:
        return {
            ...
            "beta_floor": HyperParam.floating("beta_floor", default=0.25, low=0.01, high=10.0, tunable=False),
            "beta_cap": HyperParam.floating("beta_cap", default=4.0, low=0.01, high=20.0, tunable=False),
            "rebalance_bars": HyperParam.integer("rebalance_bars", default=1, low=1, high=10080, tunable=False),
    ...
    def set_state(self, state: dict[str, Any]) -> None:
        ...
        self._latest_close = {str(k): float(v) for k, v in dict(state.get("latest_close") or {}).items() if safe_float(v) is not None}
    ...
    def _emit_pair_leg(self, pair: _PairState, event_time: Any, symbol: str, signal_type: str, close: float | None, zscore: float, reason: str, leg: str, beta: float) -> None:
        metadata = _target_metadata(
        ...
        _emit(self.events, strategy_id="pairs_spread_mean_reversion", symbol=symbol, event_time=event_time, signal_type=signal_type, strength=max(0.25, min(3.0, abs(zscore) / max(self.entry_z, _EPS))), price=close, metadata=metadata)

    def _exit_pair(self, pair: _PairState, event_time: Any, zscore: float, reason: str) -> None:
        self._emit_pair_leg(pair, event_time, pair.symbol_a, "EXIT", self._latest_close.get(pair.symbol_a), zscore, reason, "a", pair.beta)
        self._emit_pair_leg(pair, event_time, pair.symbol_b, "EXIT", self._latest_close.get(pair.symbol_b), zscore, reason, "b", pair.beta)
        pair.mode = "OUT"
        pair.entry_z = None
        pair.bars_held = 0
    ...
    def _enter_pair(self, pair: _PairState, event_time: Any, mode: str, zscore: float) -> None:
        ...
        self._emit_pair_leg(pair, event_time, pair.symbol_a, a_signal, self._latest_close.get(pair.symbol_a), zscore, "spread_entry", "a", pair.beta)
        self._emit_pair_leg(pair, event_time, pair.symbol_b, b_signal, self._latest_close.get(pair.symbol_b), zscore, "spread_entry", "b", pair.beta)
        pair.mode = mode
        pair.entry_z = zscore
        pair.bars_held = 0
    ...
    def _evaluate_pairs(self, event_time: Any, event_key: str = "") -> None:
        ...
        for pair in self._pairs.values():
            ...
            beta = _rolling_beta(pair.closes_a, pair.closes_b, self.hedge_window, self.beta_floor, self.beta_cap)
            if beta is None:
                beta = pair.beta
            pair.beta = beta
...
class CrossSectionalShortTermReversalStrategy(Strategy):
    ...
    def get_param_schema(cls) -> dict[str, HyperParam]:
        return {
            ...
            "signal_threshold": HyperParam.floating("signal_threshold", default=0.45, low=0.0, high=20.0),
            ...
            "max_trend_efficiency": HyperParam.floating("max_trend_efficiency", default=0.55, low=0.0, high=1.0),
    ...
    def _score_symbol(self, symbol: str) -> tuple[float, float, float] | None:
        ...
        eff = trend_efficiency(item.closes, window=max(self.lookback_bars, min(self.vol_window, self.lookback_bars * 2)))
    ...
    def _emit_exit(self, symbol: str, item: _CrossSectionalState, event_time: Any, reason: str) -> None:
        price = item.closes[-1] if item.closes else None
        _emit(self.events, strategy_id="cross_sectional_short_term_reversal", symbol=symbol, event_time=event_time, signal_type="EXIT", price=price, metadata={"strategy": "CrossSectionalShortTermReversalStrategy", "reason": reason})
        item.mode = "OUT"
        item.entry_price = None
        item.bars_held = 0
    ...
    def _rebalance(self, event_time: Any) -> None:
        ...
        if self._tick % self.rebalance_bars:
            ...
            return
        long_rows = [row for row in sorted(rows, reverse=True) if row[0] >= self.signal_threshold][: self.max_longs]
        short_rows = []
        if self.allow_short:
            short_rows = [row for row in sorted(rows) if row[0] <= -self.signal_threshold][: self.max_shorts]
        ...
        for symbol, item in self._state.items():
            ...
            if price is not None and self.stop_loss_pct > 0.0:
                stop_loss = price * (1.0 - self.stop_loss_pct if target_mode == "LONG" else 1.0 + self.stop_loss_pct)
            metadata = _target_metadata(
            ...
            _emit(self.events, strategy_id="cross_sectional_short_term_reversal", symbol=symbol, event_time=event_time, signal_type=target_mode, strength=max(0.25, min(3.0, abs(score) / max(self.signal_threshold, _EPS))), price=price, stop_loss=stop_loss, metadata=metadata)
