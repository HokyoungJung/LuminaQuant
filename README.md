...
Extended Binance research universe (private repo): `src/lumina_quant/research_universe.py` now records the current side-effect-free snapshot of 10 core crypto + 75 Binance USD-M `TRADIFI_PERPETUAL` symbols (85 requested symbols total). Research bar coverage is local/private: the 2026-06-05 backfill report loaded 85/85 symbols through `2026-06-05T12:00:00`, while newly listed TradFi symbols are still excluded from tuning/allocation/live promotion until train-split bars exist. The monthly-refit runner records requested/loaded/missing symbols instead of failing, so expanded listings can enter automatically after staged backfill. Adding or collecting research bars for the symbols does not approve real-money trading. See `docs/RUNBOOK_1Y_1S_LOCAL.md` and `docs/research_note/research_note.md`.
